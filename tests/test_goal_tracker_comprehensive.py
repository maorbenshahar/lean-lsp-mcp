...
GROUND_TRUTH = {
    ...
    "gt_sorry_with_sorry_deps": (True, ["gt_sub_sorry_a", "gt_sub_sorry_b"], "explicit sorry + sorry'd deps"),
    # 16. Querying a sorry leaf directly
    "gt_leaf_sorry_self":    (True,  ["gt_leaf_sorry_self"],       "direct sorry, queried directly"),
    # 17. Private sorry dep + own sorry
    "GtPrivate.pub_with_priv_dep": (True, [], "public thm with private sorry dep + own sorry"),
...
GROUND_TRUTH_IMPORT = {
    ...
    "gtImport_uses_sorry_val":   (True,  ["gtBase_sorry_val"],        "cross-file sorry dep via rfl"),
    "gtImport_uses_clean":       (False, [],                          "cross-file clean dep"),
    "gtImport_chain":            (True,  ["gtBase_chain_sorry"],       "cross-file chain to sorry"),
    "gtImport_uses_ns_sorry":    (True,  ["GtBaseNs.ns_sorry_def"],   "cross-file namespaced sorry"),
    "gtImport_uses_priv_chain":  (True,  [],                          "cross-file private sorry chain"),
    "gtImport_uses_sorry_inst":  (True,  ["gtBase_sorry_instance"],   "cross-file sorry instance"),
    "gtImport_mixed":            (True,  ["gtBase_sorry_val", "gtImport_local_sorry"], "mixed local + cross-file sorry"),
    ...
    "GtImportNs.ns_uses_base_sorry": (True, ["gtBase_sorry_val"],    "namespaced cross-file sorry"),
    "GtImportNs.ns_local_sorry": (True,  ["GtImportNs.ns_local_sorry"], "namespaced local sorry"),
    "gtImport_where":            (True,  ["gtBase_sorry_val"],         "where clause with cross-file sorry"),
    "gtImport_let_sorry":        (True,  ["gtBase_sorry_val"],        "let binding referencing sorry'd def"),
}
...
async def test_cross_file_verify_base(
    ...
    async with mcp_client_factory() as client:
        for decl, (has_sorry, _, desc) in GROUND_TRUTH_BASE.items():
            result = await client.call_tool(
                "lean_verify",
                {"file_path": str(base_file), "theorem_name": decl, "scan_source": False},
            )
            data = result_json(result)
            axioms = data["axioms"]
            got_sorry = "sorryAx" in axioms
...
async def test_cross_file_verify_import(
    ...
    async with mcp_client_factory() as client:
        for decl, (has_sorry, _, desc) in GROUND_TRUTH_IMPORT.items():
            result = await client.call_tool(
                "lean_verify",
                {"file_path": str(import_file), "theorem_name": decl, "scan_source": False},
            )
            data = result_json(result)
            axioms = data["axioms"]
            got_sorry = "sorryAx" in axioms
