...
_RG_DECL_PATTERN = (
    r"^\s*(?:private\s+|protected\s+|noncomputable\s+|nonrec\s+)?(?:theorem|lemma|def|instance)\s"
)
...
def _measure_proofs(
    ...
    for idx, (decl_line_1, keyword, name) in enumerate(sorted_decls):
        ...
        have_count = sum(1 for l in proof_lines if re.match(r"\s+have\b", l))
        calc_count = sum(1 for l in proof_lines if re.match(r"\s+calc\b", l))
